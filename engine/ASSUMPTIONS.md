...
| A-011 | 2026-07-10 | Plan price is immutable: `PATCH /finance/plans/:id` accepts only name/status; price changes = archive + create new plan | Keeps enrollment/invoice price history intact (history-over-overwrite rule) |
| A-012 | 2026-07-10 | Data access stays on supabase-js repositories; **Prisma is NOT adopted** despite appearing in the Sprint 1 ownership list | Migrating ORM mid-sprint adds risk without functional gain; repository layer isolates the choice — swapping to Prisma later touches only `src/repositories/`. PO can veto |
| A-013 | 2026-07-10 | New members enter via `POST /people` with existing `user_id` or email invite (`auth.admin.inviteUserByEmail` — Supabase sends the password-creation link) | Matches FRONTEND_ZONES onboarding ("Invites trigger a password creation screen"); requires Supabase email config to work E2E |
