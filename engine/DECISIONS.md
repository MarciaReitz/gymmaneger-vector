...
| D-024 | 2026-07-10 | Prisma adopted gradually (PO approval): `prisma/schema.prisma` mirrors the SQL migrations 1:1 (typed models, drift check via `db pull`); supabase-js repositories stay — no rewrite of working code | PO: refactor only where clear maintenance gain; repository layer isolates any future swap |
| D-025 | 2026-07-10 | List conventions v1.2: `limit`/`offset`/`sort`/`order` with per-endpoint sort whitelists; responses carry a `page` block; invalid values → 422 | Whitelists prevent sort-column injection; additive change |
| D-026 | 2026-07-10 | Structured JSON logging + `x-request-id` on every response; global 404/onError in the standard envelope; path ids validated as uuid at the gateway | Production-ready observability without new dependencies; no raw 500s from malformed input |
