...
| D-018 | 2026-07-09 | RLS grants frontend read-only access; all mutations via API with service role after explicit session→membership→permission checks (single client-write exception: post_reactions) | Keeps one enforcement path for business rules while allowing cheap RLS-protected reads |
